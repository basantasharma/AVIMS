    ...
    public function registerSubscriber(Request $request)
    {
        ...
        if($request['subscriber_type'] === 'home')
        {
            $request->validate([
                ...
                // 'middle_name' => 'required|string|max:50', // this is optional
        ...
        $request->validate([
            ...
            'portal_username' => 'required|string|unique:subscribers_details,portal_username|max:50',
            'portal_password' => 'required|string|max:50',
            'email' => 'required|string|unique:subscribers_details,email|max:75',
        ...
        if($user)
        {
            if($request['connection_type'] == 'pppoe')
            {
                ...
                if($radcheck)
                {
                    $internetId = $request->internet_package;
                    $request['attribute'] = 'Expiration';
                    $packageDetails = InternetPackages::get()->where('id', $internetId)->where('created_by', \Auth::user()->organization);
                    $expiration = now()->addMonths($packageDetails[1]['service_duration']);
                    // dd($expiration->format('d M Y'));
                    $request['value'] = $expiration->format('d M Y');

                    $radcheck1 = RadCheck::create($request->only('username', 'attribute', 'op', 'value'));
                    if($radcheck1)
                    {
                        $uploadBandwidth = $packageDetails[1]['service_upload_bandwidth'].'000k';
                        $downloadBandwidth = $packageDetails[1]['service_download_bandwidth'].'000k';

                        $request['attribute'] = 'Mikrotik-Rate-Limit';
                        $request['value'] = $uploadBandwidth.'/'.$downloadBandwidth;
                        $radcheck2 = RadReply::create($request->only('username', 'attribute', 'op', 'value'));
                        if($radcheck2)
                        {
            ...
            if($subscriberService)
            {
                if($request['iptv_package'])
                {
                    ...
                    $subscriberService1 = SubscriberService::create($request->all());
                    if($subscriberService1)
                    {
                        return redirect()->back()->with('success', "User Registered successfully");
                        // dd('user, subscriberService added');
                    }
                    return redirect()->back()->with('success', "User Registered successfully")->with('failed', 'IPTV Subscription adding failed');
                }
                // return redirect()->back()->with('success', 'User Registered successfully');
            }
            else
            {
                return redirect()->back()->with('success', "user Registered successfully")->with('failed', 'Internet Subscription adding failed');
            }
            return redirect()->back()->with('success', "user Registered successfully");


            // photo save bhayo or bhayena check garna baki
            ...
            // else
            // {
            //     return redirect()->back()->with('failed', 'failed to save identity proof photo');
            // }
        }
        return redirect()->back()->with('falied', 'User registration Failed');
